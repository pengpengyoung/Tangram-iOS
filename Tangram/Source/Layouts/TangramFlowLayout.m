...
//    });
    ...
    if (![self.superview isKindOfClass:[TangramView class]] && [self.superview conformsToProtocol:@protocol(TangramLayoutProtocol)]) {
        //如果是subLayout要撑开，走额外的逻辑
        //Model 暂时取不到，传空
        [(UIView<TangramLayoutProtocol> *)(self.superview) heightChangedWithElement:self model:nil];
    }
    if ([self.tangramView isKindOfClass:[TangramView class]]) {
        //NSLog(@"relayout invoke time in flowlayout ： %lf ",[[NSDate date] timeIntervalSince1970]);
        [self.tangramView reLayoutContent];
    }
}
